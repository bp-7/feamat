function [solution] = solver_heat_RB_rk(fespace,V, t0,T,dt,fun,u0,mu,dirichlet_functions, dirichlet_dt, method,varargin)
integrate_f = 1;
if (nargin >= 12)
    opts = varargin{1};
...
while (T-t>dt/2)
    ...
    for j = 1:nstages
        ...
        if j == 1
            rhs = V'*f - A_N*u_int_N - V'*(A*ug) - V'*M*ug_dt;
        else
            rhs = V'*f - A_N*(u_int_N + dt*(coeffs.alpha(j,1:j-1)*stages(:,1:j-1)')') - V'*A*ug - V'*M*ug_dt;
        end
