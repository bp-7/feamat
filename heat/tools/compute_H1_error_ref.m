function err = compute_H1_error_ref(fespace,values,values_ref)
% Compute H1 error with respect to reference solution
% input=
...
if (~strcmp(fespace.mesh.type,'structured'))
    for i = 1:n_elements
        ...
        for j = 1:n_gauss
            ...
            value_in_gp_grad_ref = [0;0];
            for k = 1:nlocalfunctions
                value_in_gp_f = value_in_gp_f + values(indices(k))*functions(k);
                value_in_gp_f_ref = value_in_gp_f_ref + values_ref(indices(k))*functions(k);
                value_in_gp_grad = value_in_gp_grad + values(indices(k))*grads(:,k);
                value_in_gp_grad_ref = value_in_gp_grad_ref + values_ref(indices(k))*grads(:,k);
            end
            err = err + dettransf*((value_in_gp_f-value_in_gp_f_ref)^2 + (value_in_gp_grad(1)-value_in_gp_grad_ref(1))^2 + (value_in_gp_grad(2)-value_in_gp_grad_ref(2))^2)*weights(j)/2;
        end
    end
else
    ...
    for i = 1:n_elements
        ...
        if (indices(2) == indices(1) + 1)
            for j = 1:n_gauss
                ...
                value_in_gp_f_ref = fespace.transffuns1{j}*values_ref(indices);
                value_in_gp_grad_ref = fespace.transfgrads1{j}*values_ref(indices);
                
                err = err + fespace.dettransf1*((value_in_gp_f-value_in_gp_f_ref)^2 + ...
                    (value_in_gp_grad(1)-value_in_gp_grad_ref(1))^2 + (value_in_gp_grad(2)-value_in_gp_grad_ref(2))^2)*weights(j)/2;
            end
        else
            for j = 1:n_gauss
                value_in_gp_f = fespace.transffuns2{j}*values(indices);
                value_in_gp_f_ref = fespace.transffuns2{j}*values_ref(indices);
                value_in_gp_grad = fespace.transfgrads2{j}*values(indices);
                value_in_gp_grad_ref = fespace.transfgrads2{j}*values_ref(indices);
                
                err = err + fespace.dettransf2*((value_in_gp_f-value_in_gp_f_ref)^2 + ...
                    (value_in_gp_grad(1)-value_in_gp_grad_ref(1))^2 + (value_in_gp_grad(2)-value_in_gp_grad_ref(2))^2)*weights(j)/2;
            end
        end
    end
